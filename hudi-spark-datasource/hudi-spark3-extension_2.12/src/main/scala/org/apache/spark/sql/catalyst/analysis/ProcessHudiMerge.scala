      ...
      case m @ MergeIntoTable(target, source, condition, matched, notMatched)
        ...
        val noMatchedActions = notMatched.map {
          case insert: InsertAction =>
            ...
            Option(HudiMergeInsertClause(insert.condition, actions, isStarAction))
        }

        val hudiMergeIntoTable = new HudiMergeIntoTable(target, source, condition, matchedActions, noMatchedActions(0))
        // try resolve
