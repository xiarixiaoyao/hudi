...
case class DeleteFromHudiTable(target: LogicalPlan, condition: Option[Expression]) extends RunnableCommand with Logging {
  override def run(sparkSession: SparkSession): Seq[Row] = {
    // now we cannot do skip index tag, so we dropped HOODIE_META_COLUMNS
    ...
    //execute delete
    HudiSQLUtils.update(df, HudiSQLUtils.buildDefaultParameter(HudiSQLUtils.getHoodiePropsFromRelation(target)),
      sparkSession, WriteOperationType.DELETE, false)
    Seq.empty[Row]
  }
}
