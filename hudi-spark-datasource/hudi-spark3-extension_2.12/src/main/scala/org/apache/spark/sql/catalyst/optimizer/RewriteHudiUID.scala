...
object RewriteHudiUID extends Rule[LogicalPlan] {
  ...
  def convertToUpdateCommand(update: UpdateTable): UpdateFromHudiTable = {
    ...
    val (cols, expressions) = assignments.map(a => a.key.asInstanceOf[NamedExpression] -> a.value).unzip
  ...
  /**
   * check whether we can merge the source data to target data directly
   */
  private def canMergeDirectly(merge: HudiMergeIntoTable): Boolean = {
    val HudiMergeIntoTable(target, source, _, matchedClauses, noMatchedClauses, _) = merge
    val checkSchemaMatched = {
      // consider hoodie table has extra fields
      val t: Int = if (target.schema.exists(_.name.equals(HoodieRecord.COMMIT_TIME_METADATA_FIELD))) HoodieRecord.HOODIE_META_COLUMNS.size() else 0
      val s: Int = if (target.schema.exists(_.name.equals(HoodieRecord.COMMIT_TIME_METADATA_FIELD))) HoodieRecord.HOODIE_META_COLUMNS.size() else 0

      val alignTargetSchema = target.schema.drop(t)
      val alignSourceSchema = source.schema.drop(s)

      if (alignSourceSchema.size >= alignTargetSchema.size) {
        if (alignSourceSchema.drop(alignSourceSchema.size - alignTargetSchema.size).zip(alignTargetSchema)
          .filter(f => f._1.name != f._2.name).size > 0) {
          false
        } else {
          true
        }
    ...
    val updateClause = matchedClauses.find(_.isInstanceOf[HudiMergeUpdateClause]).headOption
    val deleteClause = matchedClauses.find(_.isInstanceOf[HudiMergeDeleteClause]).headOption
    val insertClause = matchedClauses.find(_.isInstanceOf[HudiMergeInsertClause]).headOption

    def checkMergeCondition(clause: Option[HudiMergeClause], isDeleteClause: Boolean = false): Boolean = {
      if (clause.isDefined) {
        (clause.get.condition.isEmpty
          || clause.get.condition.get.references.intersect(target.outputSet).isEmpty) && (isDeleteClause || clause.get.isStarAction)
      } else {
        true
      }
    }

    checkMergeCondition(updateClause) &&
      checkMergeCondition(deleteClause, true) &&
      checkMergeCondition(insertClause) &&
      checkSchemaMatched
  }
  ...
  private def addHoodieSpecialColumn2Trim(trimmed: mutable.HashSet[StructField], target: LogicalPlan): Unit = {
    val tablePath = HudiSQLUtils.getTablePathFromRelation(target)
  ...
  def convertToMergeCommand(merge: HudiMergeIntoTable): MergeIntoHudiTable = {
    ...
    if (canMergeDirectly(merge)) {
      MergeIntoHudiTable(target, source, joinCondition, matchedClauses, noMatchedClauses, finalSchema, true, null)
    } else {
      ...
      MergeIntoHudiTable(target, source, joinCondition, processedMatched,
        processedNotMatched, finalSchema, false, StructType(trimmedStructFields.toList))
    }
  }
